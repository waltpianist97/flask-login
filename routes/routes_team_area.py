...
def team_profile(team_id):
    ...
    # Solo il leader ha diritto di modificare le impostazioni di team
...
def manage_team(team_id):
    ...
    team_members = [{"user": user, "role": user.get_role_in_team(
        team_id=team_id), "team_id": team_id} for user in team.users]
    my_role_in_team = current_user.get_role_in_team(team_id=team_id)
    requests_to_join = RequestsToJoinTeam.query.filter_by(
        team_id=team_id).all()
    requests_to_join = [{"id": request_to_join.id, "user": User.query.get(
        request_to_join.user_id)} for request_to_join in requests_to_join]
...
def manage_trips(team_id):
    ...
    my_role_in_team = current_user.get_role_in_team(team_id=team_id)
    approved_trips = [{"user": trip.get_user(), "trip": trip}
                      for trip in trips if trip.is_approved]
    non_approved_trips = [{"user": trip.get_user(), "trip": trip}
                          for trip in trips if not trip.is_approved]
...
def approve_trip(trip_id):
    ...
    send_email_utility(
        "Approvazione giro", f"Il tuo giro: {trip.tripname} e' stato approvato da {current_user.username}", AUTO_MAIL, approved_member.email)
    send_email_utility("Registrazione nuovo giro",
                       f"Il giro: {trip.tripname} di {approved_member.username} e' stato registrato", AUTO_MAIL, other_members_emails)
...
def change_role(team_id, user_id):
    ...
    if user_role_in_team != role:
        ...
        send_email_utility(
            'Cambio di ruolo', f"Il tuo ruolo nel {team_name} e' stato cambiato da {user_role_in_team} a {role}!", AUTO_MAIL, user.email)
...
def team_home(team_id):
    ...
    my_request_to_join_team = RequestsToJoinTeam.query.filter_by(
...
def member_home(user_id, team_id):
    ...
    if current_user in team.users:
        my_role_in_team = TeamUserAssociation.query.filter(and_(
            TeamUserAssociation.user_id == current_user.id, TeamUserAssociation.team_id == team_id)).first().role
    else:
        my_role_in_team = None

    trips_in_team = Trip.query.filter(
        and_(Trip.user_id == user_id, Trip.team_id == team_id)).all()
    stat = {}
    stat["average_speed"] = db.session.query(Trip, func.avg(Trip.speed)).group_by(
        Trip.user_id).filter_by(user_id=member.id, team_id=team.id).all()
    if stat["average_speed"]:
        stat["average_speed"] = round(stat["average_speed"][0][1], 2)
    stat["maximum_elevation"] = db.session.query(Trip, func.max(Trip.elevation)).group_by(
        Trip.user_id).filter_by(user_id=member.id, team_id=team.id).all()
    if stat["maximum_elevation"]:
        stat["maximum_elevation"] = stat["maximum_elevation"][0][1]
    stat["total_distance"] = db.session.query(Trip, func.sum(Trip.distance)).group_by(
        Trip.user_id).filter_by(user_id=member.id, team_id=team.id).all()
    if stat["total_distance"]:
        stat["total_distance"] = round(stat["total_distance"][0][1], 2)
    stat["activities"] = len(Trip.query.filter_by(
        user_id=member.id, team_id=team.id).all())
...
def non_member_home(user_id):
    ...
    last_trips = Trip.query.order_by(desc(Trip.recorded_on)).filter_by(
        user_id=user_id).limit(3).all()
    if last_trips is None:
        last_trips = []

    stat = {}
    stat["average_speed"] = db.session.query(Trip, func.avg(Trip.speed)).group_by(
        Trip.user_id).filter_by(user_id=user_id).all()
    if stat["average_speed"]:
        stat["average_speed"] = round(stat["average_speed"][0][1], 2)
    stat["maximum_elevation"] = db.session.query(Trip, func.max(
        Trip.elevation)).group_by(Trip.user_id).filter_by(user_id=user_id).all()
    if stat["maximum_elevation"]:
        stat["maximum_elevation"] = stat["maximum_elevation"][0][1]
    stat["total_distance"] = db.session.query(Trip, func.sum(
        Trip.distance)).group_by(Trip.user_id).filter_by(user_id=user_id).all()
    if stat["total_distance"]:
        stat["total_distance"] = round(stat["total_distance"][0][1], 2)
...
def decide_on_enrollment(request_id, accept):
    ...
    if accept == "Yes":
        ...
        send_email_utility(
            f"La tua richiesta a {team.name}", f"La tua richiesta di unirti al team {team.name} e' stata approvata!", AUTO_MAIL, user.email)
    else:
        send_email_utility(
            f"La tua richiesta a {team.name}", f"La tua richiesta di unirti al team {team.name} e' stata rifiutata!", AUTO_MAIL, user.email)
