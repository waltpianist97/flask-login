...
class StravaIO():
    ...
    def get_logged_in_athlete_activities(self, after=0, list_activities=None):
        ...
        _fetched = self.activities_api.get_logged_in_athlete_activities(
            after=after)
        if len(_fetched) > 0:
            print(
                f"Fetched {len(_fetched)}, the latests is on {_fetched[-1].start_date}")
    ...
    def local_activities(self, athlete_id):
        ...
        """
        dir_activities = os.path.join(
            dir_stravadata(), f"activities_{athlete_id}")
    ...
    def get_activity_streams(self, id, athlete_id, local=True):
        ...
        """
        if local:
            dir_streams = os.path.join(
                dir_stravadata(), f"streams_{athlete_id}")
        ...
        api_response = self.streams_api.get_activity_streams(
            id, keys, key_by_type=True)
...
class Streams():
    ...
    ACCEPTED_KEYS = ['time', 'distance', 'altitude', 'velocity_smooth', 'heartrate',
                     'cadence', 'watts', 'temp', 'moving', 'grade_smooth', 'lat', 'lng']
...
def run_server_and_wait_for_token(port, client_id, client_secret):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        if url.path == "/authorization_successful":
            ...
            data = r.json()
            logger.debug(
                f"Authorized athlete: {data.get('access_token', 'Oeps something went wrong!')}")
        else:
