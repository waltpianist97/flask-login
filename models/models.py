...
class User(db.Model, UserMixin, AdminMixin):
    ...
    join_requests = relationship(
        "RequestsToJoinTeam", back_populates="user", cascade="all, delete-orphan")
    ...
    def get_role_in_team(self, team_id):
        user_associated = TeamUserAssociation.query.filter_by(
            team_id=team_id, user_id=self.id).first()
        if user_associated:
    ...
    def group_user_trips_by_team(self):
        ...
        for team in self.teams:
            trips_in_team = [
                trip for trip in self.trips if trip.team_id == team.id]
            result.append(
                {"team_name": team.name, "team_id": team.id, "trips_by_team": trips_in_team})
        return result
...
class Trip(db.Model):
    ...
    placements = relationship(
        "PlacementsInTrip", backref="trip", cascade="all, delete-orphan")
    ...
    def calculate_score(v, dx, dz, pr, np, piazzamenti):
        ...
        def punteggio_finale_o(v, dx, dz, pr): return dx*l*(dz*d+0.25)*(0.7+0.15*(
            pr-3)) if v <= 25.0 else dx*l*(dz*d+0.25)*(0.7+0.15*(pr-3))*(1+0.05*(v-25.0))
        def punteggio_finale_f(punteggio_finale_o, np, piazzamenti): return int(round(
            (punteggio_finale_o + (np-1)/10)*100, 0))+11*len(piazzamenti)-sum(piazzamenti)
...
class Team(db.Model):
    id = Column(Integer, primary_key=True)
    name = Column(String(140))
    trips = relationship('Trip', backref='team',
                         lazy='dynamic', cascade="all, delete-orphan")
    users = relationship(
        'User', secondary="team_user_association", back_populates='teams')
    join_requests = relationship(
        "RequestsToJoinTeam", back_populates="team", cascade="all, delete-orphan")
    description = Column(String(140))
    ...
    def add_member(self, member: User, role: str, join_date: datetime = None):
        t_u_association = TeamUserAssociation(
            team_id=self.id, user_id=member.id, role=role, join_date=join_date, current_ranking=len(self.users)+1)
        db.session.add(t_u_association)
        db.session.commit()

    def set_member_ranking(self, member: User, current_ranking: int):
        t_u_association = TeamUserAssociation.query.filter_by(
            user_id=member.id, team_id=self.id).first()
        previous_ranking = t_u_association.current_ranking
        if previous_ranking != current_ranking:
            send_email_utility(
                'Cambio ranking', f"Il tuo ranking è cambiato nel team {self.name}! Sei passato dal {previous_ranking}° al {current_ranking}° posto!", AUTO_MAIL, member.email)

        t_u_association.current_ranking = current_ranking
        db.session.commit()

    def ranking_builder(self):
        members_by_team = User.query.filter(User.id.in_(
            [member.id for member in self.users])).all()
        ranking_list = []
        for user_by_team in members_by_team:
            all_scores_by_user = Trip.query.filter_by(
                user_id=user_by_team.id, team_id=self.id, is_approved=True).all()
            tot_score_by_user = sum(
                [score_by_user.score for score_by_user in all_scores_by_user])
            ranking_list.append({"user_id": user_by_team.id, "user": user_by_team.username,
                                "total score": tot_score_by_user, "user_obj": user_by_team})
        ranking_list = list(
            enumerate(sorted(ranking_list, key=lambda x: x['total score'], reverse=True)))
    ...
    def get_leaders(self):
        leaders = User.query.join(TeamUserAssociation).filter(
